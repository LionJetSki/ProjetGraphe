...
public class Main {
	public static void main(String[] args) throws NumberFormatException, FileNotFoundException, IOException {
		Arc df = new Arc();
		IGraphe g = importer("g-10-2.txt", df);
		System.out.print(g);
		System.out.println("debut fin : "+ df.getSource() + " ==> "+ df.getDestination());
    PCCDijkstra test = new PCCDijkstra(g);
    test.Resultat(df.getSource(),df.getDestination());
...
public static void verifierGraphes() throws FileNotFoundException {
	...
	File dir = new File(dirStr);
	  ...
	  if (directoryListing != null) {
	    for (File child : directoryListing) {
	      ...
	      System.out.println("debut et fin du chemin � trouver : "+ df.getSource()+ " ==> "+ df.getDestination()+"\n");
	    }
	  } else {
	    System.out.println("Erreur : "+ dirStr + " n'est pas un r�peroire");
	  }
}
	...
	private static IGraphe importer(File file, Arc df) throws FileNotFoundException {
		...
		while (sc.hasNextLine()) {
			...
			else {// la derniere ligne n'est pas un arc mais le debut et la fin du chemin � trouver
				df.set(a);
			}
		}
		sc.close();
